...
{
    [Cmdlet(VerbsLifecycle.Disable, "Device", DefaultParameterSetName = "ByInstanceId", ConfirmImpact = ConfirmImpact.High, SupportsShouldProcess = true)]
    [OutputType(typeof(Device))]
    public class DisableDeviceCmdlet : PSCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            ...
            foreach (var device in devices)
            {
                try
                {
                    ...
                    {
                        DeviceManagementLib.DeviceManagementLib.SetDeviceState(device.InstanceId, DeviceManagementLib.Win32.DICS.DICS_DISABLE);
                    }
                }
                catch(Exception ex)
                {
                    WriteError(new ErrorRecord(ex, "ErrorDisablingDevices", ErrorCategory.WriteError, null));
                }
            }
        }
